...
package cpu.components {
  ...
  object Instructions {
    ...
    // R type
    // add word
    // ADD rd, rs, rt  : add rs, rt to rd
    //TRAP: yes. trap if there is an overflow
    def ADDI = BitPat("b001000??????????????????????????")

    // I type
    // add immediate word
    // addi rt, rs, immediate
    // trap: yes. Trap is there is an overflow
    def ADDIU = BitPat("b001001??????????????????????????")

    // I type
    // add immediate unsigned word
    // addiu rt, rs, immediate
    // trap: no. Don't trap is there is an overflow
    def ADDU = BitPat("b000000???????????????00000100001")

    // R type
    // add unsigned word
    // ADDU rd, rs, rt  : add unsigned rs, rt to rd
    //TRAP: no. Don't trap if there is an overflow
    ...
    // I type
    // branch if equal
    // beq rs, rt, offset(jump location)
    // trap: no
    def BGEZ = BitPat("b000001?????00001????????????????") // branch if equal to or greater than zero
    // I type
    ...
    // I type
    // branch if greater than or equal to zero and link ( place ra in register[31] )
    // WARNING: reg[31] cannot be rs!!
    // bgezal rs, offset
    // trap: no
    ...
    // WARNING: rs cannot be 31
    // bltzal rs, offset
    // trap: no
    def BNE = BitPat("b000101??????????????????????????")

    // I type
    // branch if rs is not equal to rt
    // bne rs, rt, offset
    // trap: no
    def BREAK = BitPat("b000000????????????????????001101")

    // R type ( according to https://stackoverflow.com/questions/53188335/what-type-is-the-break-instruction-on-mars-mips-assembly)
    ...
    // R type
    // divide word,  rs/rt both signed, quotient -> reg LO, remainder -> reg HI
    // div rs,rt
    // trap: no, return anything if rt == 0
    def DIVU = BitPat("b000000??????????0000000000011011")

    // R type
    // divide word,  rs/rt both unsigned, quotient -> reg LO, remainder -> reg HI
    // div rs,rt
    // trap: no, return anything if rt == 0
    ...
    // eret
    // trap: yes. co-processor unusable exception
    def J = BitPat("b000010??????????????????????????")

    // J type
    // Jump immediate offset
    // j immediate
    // trap: no
    def JAL = BitPat("b000011??????????????????????????")

    // J type
    // Jump and Link, put the ra into reg[31]
    // jal immediate
    // trap: no
    def JALR = BitPat("b000000?????00000??????????001001")

    // J type
    // Jump and Link Register, load the ra into reg[31](ra) if rd not specified. Jump to rs
    // jalr rd, rs
    // trap: no

    ...
    when(opIsLoad(io.ins(memSlot).op)) {
      ...
      io.out(memSlot).write.data := MuxLookup(
        ...
        Seq(
          ...
          MEM_LH  -> Mux(io.ins(memSlot).addrL2(1), Util.signedExtend(readData(31, 16)), Util.signedExtend(readData(15, 0))),
          MEM_LHU -> Mux(io.ins(memSlot).addrL2(1), Util.zeroExtend(readData(31, 16)), Util.zeroExtend(readData(15, 0))),
          MEM_LW  -> readData,
          MEM_LWL -> MuxLookup(
            io.ins(memSlot).addrL2,
            io.ins(memSlot).write.data,

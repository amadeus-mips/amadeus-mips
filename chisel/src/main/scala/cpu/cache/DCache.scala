...
class DCache(wayAmount: Int = 2, numOfSets: Int = 128) extends Module {
  val indexLen = log2Ceil(numOfSets) // index宽度
  ...
  /** valid(way)(index) */
  val valid = RegInit(VecInit(Seq.fill(wayAmount)(VecInit(Seq.fill(numOfSets)(false.B)))))

  /** dirty(way)(index) */
  val dirty = RegInit(VecInit(Seq.fill(wayAmount)(VecInit(Seq.fill(numOfSets)(false.B)))))

  /** LRU(index) */
  val LRU = RegInit(VecInit(Seq.fill(numOfSets)(0.U((wayAmount - 1).W))))
  ...
  val tagBanks = for (i <- 0 until wayAmount) yield {
    val bank = Module(new SinglePortBank(numOfSets, tagLen, syncRead = false, true))
    bank.io.we := tagWe(i)
  ...
  val dataBanks = for {
    ...
    val bank = Module(new SimpleDualPortMaskBank(numOfSets, maskW, maskN = bankSize))

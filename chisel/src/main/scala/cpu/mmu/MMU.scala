...
class MMU(implicit conf: CPUConfig) extends Module {
  ...
  tlb.io.query(0).vAddr := io.in.rInst.addr(31, 12)
  ...
  io.out            <> io.in
  io.out.rInst.addr := Cat(0.U(3.W), tlb.io.result(0).pageInfo.pfn(16,0), io.in.rInst.addr(11, 0))
  io.out.rInst.enable := io.in.rInst.enable &&

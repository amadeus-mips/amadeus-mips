...
class TrueDualPortRam(depth: Int, width: Int, byteAddressable: Boolean)(implicit
  ...
  if (cpuCfg.build) {
    ...
    assert(io.portA.writeVector.orR || !io.portA.portEnable, "when write port enable is high, write vector cannot be all 0")
    if (byteAddressable) {
      val bank = SyncReadMem(depth, Vec(width / 8, UInt(8.W)))
      when(io.portA.portEnable) {
        when(io.portA.writeVector.orR) {
          bank.write(io.portA.addr, io.portA.writeData.asTypeOf(Vec(width / 8, UInt(8.W))), io.portA.writeVector.asBools())
        } otherwise {
      ...
      when(io.portB.portEnable) {
        when(io.portB.writeVector.orR) {
          bank.write(io.portB.addr, io.portB.writeData.asTypeOf(Vec(width / 8, UInt(8.W))), io.portB.writeVector.asBools())
        } otherwise {

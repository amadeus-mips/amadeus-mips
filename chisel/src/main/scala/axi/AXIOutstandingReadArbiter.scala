...
/**
  ...
  * @note
  *   please change the ordering of the ports
  */
class AXIOutstandingReadArbiter(capacity: Int = 64) extends Module {
  ...
  val readQueue = Module(new Queue(UInt(log2Ceil(masterCount).W), capacity, pipe = true, flow = true))
